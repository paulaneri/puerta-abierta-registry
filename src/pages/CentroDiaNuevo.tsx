...
const CentroDiaNuevo = () => {
  ...
  const [nuevaMujer, setNuevaMujer] = useState({ nombre: '', apellido: '', apodo: '', fechaNacimiento: '' });
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        // Cargar trabajo de campo del día automáticamente
        cargarTrabajoCampoDia(registroActual.fecha);
      } catch (error) {
        console.error('Error cargando datos:', error);
        // Fallback a datos síncronos si falla
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultado = await centroDiaStore.agregarRegistro(registroCompleto);
  ...
  const agregarMujer = (mujer: Mujer) => {
    console.log('Intentando agregar mujer:', mujer);
    console.log('Mujeres ya asistiendo:', registroActual.mujeresAsistieron);
    
    const yaEsta = registroActual.mujeresAsistieron.some(m => {
      const existe = m.mujer.id === mujer.id;
      console.log(`Comparando ${m.mujer.id} === ${mujer.id}:`, existe);
      return existe;
    });
    
    console.log('¿Ya está agregada?:', yaEsta);
    ...
    if (!yaEsta) {
      setRegistroActual(prev => {
        ...
        console.log('Nuevo estado después de agregar:', nuevoEstado.mujeresAsistieron);
        return nuevoEstado;
      });
      setHasChanges(true);
    }
  ...
  const agregarNuevaMujer = async () => {
    if (nuevaMujer.nombre.trim() && nuevaMujer.apellido.trim()) {
      const nuevaMujerCompleta: Mujer = {
        ...
        fechaNacimiento: nuevaMujer.fechaNacimiento || '',
        nacionalidad: '',
        telefono: '',
        email: '',
        direccion: '',
      ...
      try {
        ...
        if (success) {
          ...
          setNuevaMujer({ nombre: '', apellido: '', apodo: '', fechaNacimiento: '' });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground">Nuevo Registro - Centro de Día</h1>
                <p className="text-muted-foreground">Registra las actividades del día</p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <DatePicker
                  date={registroActual.fecha ? (() => {
                    const [year, month, day] = registroActual.fecha.split('-').map(Number);
                    ...
                    <div className="mb-4 p-3 border rounded-lg bg-white">
                      ...
                      <div className="grid grid-cols-1 gap-2 mb-2">
                        ...
                        <Input
                          ...
                          type="date"
                          value={nuevaMujer.fechaNacimiento || ''}
                          onChange={(e) => setNuevaMujer(prev => ({ ...prev, fechaNacimiento: e.target.value }))}
                        />
                        <Input
